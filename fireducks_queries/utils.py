...
def _read_ds(path: str) -> PandasDF:
    if FILE_TYPE == "parquet":
        ...
        df = pd.read_parquet(f"{path}.{FILE_TYPE}")
...
def test_results(q_num: int, result_df: PandasDF):
    with CodeTimer(name=f"Testing result of pandas Query {q_num}", unit="s"):
        ...
        assert result_df.shape == answer.shape
...
def run_query(q_num: int, query: Callable):
    @linetimer(name=f"Overall execution of fireducks Query {q_num}", unit="s")
    def run():
        with CodeTimer(name=f"Get result of fireducks Query {q_num}", unit="s"):
        ...
        if LOG_TIMINGS:
            append_row(
                solution="fireducks", version=pd.__version__, q=f"q{q_num}", secs=secs
            )
        else:
